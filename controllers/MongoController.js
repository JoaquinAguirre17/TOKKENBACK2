...
export const createWebOrderMP = async (req, res) => {
  ...
  try {
    ...
    console.log("🛒 PEDIDO WEB:", req.body);
    ...
    const items = productos.map(item => {
      ...
      if (stock < quantity) {
        ...
        throw new Error(
          `Stock insuficiente para ${db.title}. Disponible: ${stock}`
        );
    ...
    if (
      ...
      return res.status(400).json({
        ...
        message:
          "El total enviado por el frontend no coincide",
    ...
    if (!process.env.MP_ACCESS_TOKEN) {
      ...
      console.log(
        "⚠️ MP_ACCESS_TOKEN no configurado."
      );
      ...
      return res.status(201).json({
        ...
        message:
          "Pedido creado en modo simulación",
    ...
    const preferenceData = {
      ...
      notification_url:
        `${process.env.BACKEND_URL}/api/orders/web-mp/webhook`,
